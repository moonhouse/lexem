    ...
    for motion_filename in motioner:
        with myzip.open(motion_filename, mode='r') as motion_file:
            ...
            html = lxml.html.fromstring(
                motion['dokumentstatus']['dokument']['html'])
            ...
            try:
                ...
                preceding_one = html.xpath(
                    "//*[@class='Section1']")[0].itersiblings(preceding=True)

                if len(html.xpath("//*[@style='-aw-sdt-tag:CC_Underskrifter; -aw-sdt-title:CC_Underskrifter']")) > 0:
                    following = html.xpath(
                        "//*[@style='-aw-sdt-tag:CC_Underskrifter; -aw-sdt-title:CC_Underskrifter']")[0].itersiblings()
                else:
                    following = html.xpath(
                        "//*[@class='Underskrifter']")[0].itersiblings()

                if len(html.xpath("//*[@class='Frslagstext']")) > 0:
                    preceding_two = html.xpath(
                        "//*[@class='Frslagstext']")[0].itersiblings(preceding=True)
                else:
                    preceding_two = iter([])

                motivering = html.xpath(
                    "//a[@name='MotionsStart']")[0].itersiblings()

                generator = chain(preceding_one, preceding_two,
                                  following, motivering)
                ...
                words = list(set(plaintext.translate(
                    str.maketrans('', '', my_punctuation)).split()))
                words = filter(lambda word: not bool(
                    re.search(r'\d', word)), words)
                missing_words = []
                for word in words:
